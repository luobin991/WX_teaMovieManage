...
var bootstrap = function ($, luoluo) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#ll_enabled').on('click', function () {
                ...
                if (luoluo.checkrow(keyValue)) {
                    luoluo.layerConfirm('是否确认要【启用】账号！', function (res) {
                        if (res) {
                            luoluo.postForm(top.$.rootUrl + '/SystemModule/SysUser/UpdateState', { keyValue: keyValue, state: 1008001 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#ll_disabled').on('click', function () {
                ...
                if (luoluo.checkrow(keyValue)) {
                    luoluo.layerConfirm('是否确认要【禁用】账号！', function (res) {
                        if (res) {
                            luoluo.postForm(top.$.rootUrl + '/SystemModule/SysUser/UpdateState', { keyValue: keyValue, state: 1008002 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            // 重置密码
            $('#ll_resetpassword').on('click', function () {
                var keyValue = $('#girdtable').jfGridValue('Id');
                if (luoluo.checkrow(keyValue)) {
                    luoluo.layerConfirm('是否确认要【重置密码】！', function (res) {
                        if (res) {
                            luoluo.postForm(top.$.rootUrl + '/SystemModule/SysUser/ResetPassword', { keyValue: keyValue }, function () {
                            });
                        }
                    });
                }
                ...
                headData: [
                        ...
                        { label: '真实名字', name: 'UserNickName', width: 150, align: 'center' },
                        {
                            label: '管理类型', name: 'UserType', width: 100, align: 'center', formatter: function (cellvalue) {
                                //return luoluo.formatDate(cellvalue, 'yyyy-MM-dd hh:mm');
