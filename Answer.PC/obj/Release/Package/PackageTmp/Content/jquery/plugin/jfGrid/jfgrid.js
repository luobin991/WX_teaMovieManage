    ...
    $.extend($.jfGrid, {
        ...
        renderPageBarinit: function ($self) {
            ...
            if (dfop.isPage && dfop.url) {
                ...
                var $pagebar = $('<div class="jfgrid-page-bar" id="jfgrid_page_bar_' + dfop.id + '"><div class="jfgrid-page-bar-info" >无显示数据</div><ul class="pagination pagination-sm" id="jfgrid_page_bar_num_' + dfop.id + '" ></ul></div>');
                $self.append($pagebar);
        ...
        renderToolBar: function ($self) {
            ...
            if (dfop.isEidt) {
                ...
                $minus.on('click', function () {
                    var index = $self.find('.jfgrid_selected_' + dfop.id + '[datapath]').attr('datapath');
                    if (index != undefined) {
                        if (dfop.rowdatas.length == 1) {
                            dfop.rowdatas.splice(index, 1);
                            dfop.rowdatas.push({});
                        }
                        else {
                            dfop.rowdatas.splice(index, 1);
                        ...
                        top.luoluo.alert.warning('请选择需要删除的行!');
                    }
                });
            ...
            if ($et.hasClass('pagebtn')) {
                ...
                if (page != dfop.pageparam.page) {
                    ...
                    $.jfGrid.ajaxLoad($self, dfop.url, dfop.param, function (res) {
                        ...
                        if (allcheck == true) {
                            $('.jfgrid-tool-cell [role="checkbox"]').not(':checked').trigger('click');
                        }
                    });
                }
            }
                    ...
                    case 'select':
                        ...
                        $label.find('.fa-ellipsis-h').on('click', function () {
                            $.jfGrid.layer({
                                ...
                                width: node.editOp.width || 400,
                                height: node.editOp.height || 400
                            },
                            function ($html) {
                                $html.find('#jfgird_select').jfGrid({
                                    headData: node.editOp.colData,
                                    url: node.editOp.url,
                                    onRenderComplete: function (rowdatas) {
                                        node.editOp.rowdatas = rowdatas;
                                    },
                                    onSelectRow: function (rowdata) {
                                        if (!!node.editOp.callback) {
                                            node.editOp.callback(rowdata, rownum, row);
                                        }
                                        $html.remove();
                                        $.jfGrid.renderData($('#' + dfop.id));
                                    }
                                });
                                if (!!node.editOp.rowdatas) {
                                    $html.find('#jfgird_select').jfGridSet('refreshdata', { rowdatas: node.editOp.rowdatas });
                                }
                                else {
                                    $html.find('#jfgird_select').jfGridSet('reload', { param: node.editOp.param });
                                }
                                $('#jfgird_select_search').on('click', function () {
                                    var data = [];
                                    var keyword = $('#jfgird_select_keyword').val();
                                    if (!!keyword) {
                                        for (var i = 0, l = node.editOp.rowdatas.length; i < l; i++) {
                                            var item = node.editOp.rowdatas[i];
                                            for (var j = 0, jl = node.editOp.colData.length; j < jl; j++) {
                                                if (item[node.editOp.colData[j].name].indexOf(keyword) != -1) {
                                                    data.push(item);
                                                    break;
                                                }
                                            }
                                        ...
                                        $('#jfgird_select').jfGridSet('refreshdata', { rowdatas: node.editOp.rowdatas });
                                    }
