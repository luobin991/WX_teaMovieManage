    ...
    $.extend(luoluo, {
        ...
        layerForm: function (op) {
            ...
            var r = top.layer.open({
                ...
                btn: dfop.btn,
                content: op.url,
                skin: dfop.btn == null ? 'll-layer-nobtn' : 'll-layer',
                success: function (layero, index) {
                    top['layer_' + dfop.id] = luoluo.iframe($(layero).find('iframe').attr('id'), top.frames);
                    layero[0].learun_layerid = 'layer_' + dfop.id;
                    //如果底部有按钮添加-确认并关闭窗口勾选按钮
                    if (!!dfop.btn && layero.find('.ll-layer-btn-cb').length == 0) {
                        layero.find('.layui-layer-btn').append('<div class="checkbox ll-layer-btn-cb" myIframeId="layer_' + dfop.id + '" ><label><input checked="checked" type="checkbox" >确认并关闭窗口</label></div>');
                    }
        ...
        layerClose: function (name, index) {
            ...
            var $IsClose = layero.find('.layui-layer-btn').find(".ll-layer-btn-cb input");
