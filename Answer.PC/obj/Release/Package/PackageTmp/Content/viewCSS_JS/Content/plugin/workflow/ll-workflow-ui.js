...
(function ($, luoluo) {
    ...
    $.lrworkflow = {
        render: function ($self) {
            ...
            $self.addClass('ll-workflow');
            // 添加工具栏
            if (!dfop.isPreview) {
                var $tool = $('<div class="ll-workflow-tool" ></div>');
                $tool[0].dfop = dfop;
                $tool.append('<a  type="cursor" class="ll-workflow-btndown" id="' + dfop.id + '_btn_cursor" title="' + dfop.nodeRemarks.cursor + '" ><b class="ico_cursor" /></a>');
                $tool.append('<a  type="direct" class="ll-workflow-btn" id="' + dfop.id + '_btn_direct" title="' + dfop.nodeRemarks.direct + '" ><b class="ico_direct"/></a>');
                var toolbtnlen = dfop.toolBtns.length;
                if (toolbtnlen > 0) {
                    $tool.append('<span></span>');
                    for (var i = 0; i < toolbtnlen; ++i) {
                        var btn = dfop.toolBtns[i];
                        $tool.append('<a  type="' + btn + '" id="' + dfop.id + '_btn_' + btn + '" class="ll-workflow-btn" title="' + dfop.nodeRemarks[btn] + '" ><b class="ico_' + btn + '"/></a>');//加入自定义按钮
                    }
                }
                ...
                $self.addClass(' ll-workflow-preview');
            }
           

            // 工作流画板(工作区域)
            $self.append('<div class="ll-workflow-work"></div>');
            var $workArea = $("<div class='ll-workflow-workinner' style='width:5000px;height:5000px'></div>")
		        .attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
            
            $self.children(".ll-workflow-work").mCustomScrollbar({ // 优化滚动条
                axis: "xy"
            });
            $self.children(".ll-workflow-work").find('.mCSB_container').append($workArea);
            ...
            var $ghost = $("<div class='ll-workflow-rsghost'></div>").attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
            $self.append($ghost);

            var $lineMove = $("<div class='ll-workflow-linemove' style='display:none'></div>");//操作折线时的移动框
            ...
            // 设置线条工具条选定线时显示的操作框
            var $lineOper = $("<div class='ll-workflow-lineoper' style='display:none'><b class='ll'></b><b class='tb'></b><b class='sl'></b><b class='x'></b></div>");
        ...
        switchToolBtn: function (dfop, type) {
            ...
            $oldBtn.removeClass('ll-workflow-btndown');
            $oldBtn.addClass('ll-workflow-btn');

            $newBtn.removeClass('ll-workflow-btn');
            $newBtn.addClass('ll-workflow-btndown');
        ...
        clickWorkArea: function (e) {
            ...
            if (!dfop.isPreview) {
                ...
                if (type == "cursor") {
                    ...
                    if (n == "svg" || (n == "DIV" && t.prop("class").indexOf("ll-workflow-workinner") > -1)) {
                ...
                if (type == 'endround') {
                    name = "结束";
                    if (dfop.hasEndround) {
                        luoluo.alert.error('只能有一个结束节点');
                        return false;
                    }
                }
                $.lrworkflow.addNode($this, dfop, { id: luoluo.newGuid(), name: name, left: X, top: Y, type: type });
            }
        },
        // 取消所有结点/连线被选定的状态
        blurItem: function (dfop) {
            if (dfop.focusId != "") {
                ...
                if ($item.prop("tagName") == "DIV") {
                    $item.removeClass("ll-workflow-nodefocus");
                    $item.find('.ll-workflow-nodeclose').hide();
                    $item.removeClass("ll-workflow-nodemark");
                }
                else {
                    ...
                    var $lineOper = $('.ll-workflow-lineoper');
                    var $lineMove = $('.ll-workflow-linemove');
        ...
        focusItem: function (id) {
            ...
            if ($item.prop("tagName") == "DIV") {
                ...
                $item.addClass("ll-workflow-nodefocus");
                $item.find('.ll-workflow-nodeclose').show();
            }
            else {//如果是连接线
        ...
        addNode: function ($workArea, dfop, node, isold) {
            ...
            if (mark == "conditionnode")
            {
                node.width = 160;
                node.height = 90;
                $node = $('<div class="ll-workflow-node item-conditionnode" id="' + node.id + '" ><div class="ll-workflow-nodeico"></div><b class="ico_' + node.type + 'div"></b><div class="ll-workflow-nodetext">' + node.name + '</div><div class="ll-workflow-nodeassemble" ></div></div>');

            }
            else if (mark != "startround" && mark != "endround") {
                $node = $('<div class="ll-workflow-node" id="' + node.id + '" ><div class="ll-workflow-nodeico"><b class="ico_' + node.type + '"></b></div><div class="ll-workflow-nodetext">' + node.name + '</div><div class="ll-workflow-nodeassemble" ></div></div>');
            }
            else {
                node.width = 52;
                node.height = 52;
                ...
                $node = $('<div class="ll-workflow-node item-' + mark + '" id="' + node.id + '" ><div class="ll-workflow-nodeico"></div><div class="ll-workflow-nodetext">' + node.name + '</div><div class="ll-workflow-nodeassemble" ></div></div>');
            }
            $node.find('.ll-workflow-nodeassemble').append('<div class="ll-workflow-nodeclose"></div>');
            $node.find('.ll-workflow-nodeassemble').append('<div class="ll-workflow-nodespot left"><div class="ll-workflow-nodespotc"></div></div>');
            $node.find('.ll-workflow-nodeassemble').append('<div class="ll-workflow-nodespot top"><div class="ll-workflow-nodespotc"></div></div>');
            $node.find('.ll-workflow-nodeassemble').append('<div class="ll-workflow-nodespot right"><div class="ll-workflow-nodespotc"></div></div>');
            $node.find('.ll-workflow-nodeassemble').append('<div class="ll-workflow-nodespot bottom"><div class="ll-workflow-nodespotc"></div></div>');
            ...
            if (node.state != undefined && (node.type == 'startround' || node.type == 'auditornode' || node.type == 'stepnode' || node.type == 'confluencenode')) {
                $node.css({ 'padding-left': '0','color':'#fff' }).find('.ll-workflow-nodeico').remove();
        ...
        updateNodeName: function ($workArea, nodeId) {
            ...
            var nodeData = $node[0].wfdata;
            $node.find('.ll-workflow-nodetext').html(nodeData.name);
        },
        initNodeEvent: function ($workArea) {
            var dfop = $workArea[0].dfop;

            //节点双击事件
            $workArea.delegate(".ll-workflow-node", "dblclick", { $workArea: $workArea }, function (e) {
                var $workArea = e.data.$workArea;
                var dfop = $workArea[0].dfop;
                var $node = $(this);
                var nodeData = $node[0].wfdata;
            ...
            if (!dfop.isPreview) {
                //绑定点击事件
                $workArea.delegate(".ll-workflow-node", "click", function (e) {
                    $.lrworkflow.focusItem(this.id);
                    return false;
                });
                //绑定右击事件
                $workArea.delegate(".ll-workflow-node", "contextmenu", function (e) {
                    //$.lrworkflow.focusItem(this.id);
                    //return false;
                });
                //绑定用鼠标移动事件(节点拖动)
                $workArea.delegate(".ll-workflow-nodeico", "mousedown", { $workArea: $workArea }, function (e) {
                    var $node = $(this).parents(".ll-workflow-node");
                    var dfop = $node[0].dfop;
                    var nodeData = $node[0].wfdata;
                    ...
                    var ev = $.lrworkflow.mousePosition(e), t = $.lrworkflow.getElCoordinate($workArea[0]);
                    var $ghost = $('#' + dfop.id).find('.ll-workflow-rsghost');
                    ...
                    $ghost.find('.ll-workflow-nodeclose').remove();
                    var X, Y;
                ...
                //绑定鼠标覆盖/移出事件
                $workArea.delegate(".ll-workflow-node", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("ll-workflow-nodemark");
                });
                $workArea.delegate(".ll-workflow-node", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("ll-workflow-nodemark");
                });

                $workArea.delegate(".ll-workflow-nodespot", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("ll-workflow-nodespotmark");
                });
                $workArea.delegate(".ll-workflow-nodespot", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("ll-workflow-nodespotmark");
                });

                //绑定连线时确定初始点
                $workArea.delegate(".ll-workflow-nodespot", "mousedown", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    var $node = $this.parents('.ll-workflow-node');
                ...
                //绑定连线时确定结束点
                $workArea.delegate(".ll-workflow-nodespot", "mouseup", { $workArea: $workArea }, function (e) {
                    var $workArea = e.data.$workArea;
                    var dfop = $workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    ...
                    if (lineStart) $.lrworkflow.addLine(dfop, { id: luoluo.newGuid(), from: lineStart.id, to: nodeData.id, sp: lineStart.position, ep: position, name: "" });
                });
                //绑定结点的删除功能
                $workArea.delegate(".ll-workflow-nodeclose", "click", function () {
                    var $node = $(this).parents('.ll-workflow-node');
                    var nodeData = $node[0].wfdata;
                    var dfop = $node[0].dfop;
        ...
        //重新设置连线的样式 newType= "sl":直线, "ll":中段可左右移动型折线, "tb":中段可上下移动型折线
        ...
        setLineM: function (id, M, noStack) {
            ...
            var dfop = $('#' + id)[0].dfop;
            lineData = $.lrworkflow.getLine(dfop, id);
    ...
    $.fn.lrworkflowGet = function () {
        var $self = $(this);
        var $workArea = $self.find(".ll-workflow-workinner");
        var dfop = $workArea[0].dfop;
