    ...
    var page = {
        ...
        bind: function () {
            ...
            $("#OldPassword").blur(function () {
                ...
                learun.httpAsyncPost(top.$.rootUrl + "/SystemModule/SysUser/ValidationOldPassword", { OldPassword: password }, function (res) {
                    if (res.code != 200) {
            ...
            $('#lr_save_btn').on('click', function () {
                ...
                if ($('#OldPassword').parent().find('.tip-success').length > 0 && $('#NewPassword').parent().find('.tip-success').length > 0 && $('#RedoNewPassword').parent().find('.tip-success').length > 0)
                {
                    ...
                    learun.layerConfirm('注：请牢记当前设置密码，您确认要修改密码？', function (res, index) {
                        if (res) {
                            $.lrSaveForm(top.$.rootUrl + '/SystemModule/SysUser/SubmitResetPassword', postData, function (res) {
                                if (res.code == 200) {
