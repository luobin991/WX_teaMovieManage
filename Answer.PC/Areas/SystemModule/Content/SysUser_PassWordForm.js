    ...
    var page = {
        ...
        bind: function () {
            ...
            $("#Verifycode_img").click(function () {
                $("#Verifycode").val('');
                $("#Verifycode_img").attr("src", top.$.rootUrl + "/SystemModule/SysUser/VerifyCode?time=" + Math.random());
            });
            // 旧密码验证
            $("#OldPassword").blur(function () {
                ...
                luoluo.httpAsyncPost(top.$.rootUrl + "/SystemModule/SysUser/ValidationOldPassword", { OldPassword: password }, function (res) {
                    if (res.code != 200) {
            ...
            $('#ll_save_btn').on('click', function () {
                ...
                {
                    var formData = $('#form').llGetFormData();
                    var postData = {
                    ...
                    luoluo.layerConfirm('注：请牢记当前设置密码，您确认要修改密码？', function (res, index) {
                        if (res) {
                            $.llSaveForm(top.$.rootUrl + '/SystemModule/SysUser/SubmitResetPassword', postData, function (res) {
                                if (res.code == 200) {
