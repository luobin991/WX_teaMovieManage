...
(function ($, luoluo) {
    ...
    $.lrCustmerFormDesigner = {
        init: function ($self, op) {
            ...
            $self.addClass('ll-custmerform-designer-layout');
            var _html = '';
            _html += '<div class="ll-custmerform-designer-layout-left"  id="ll_custmerform_compont_list_' + dfop.id + '"></div>';

            _html += '<div class="ll-custmerform-designer-layout-center">';
            _html += '<div class="ll-custmerform-designer-layout-header">';
            _html += '<div class="ll-custmerform-designer-tabs" id="ll_custmerform_designer_tabs_' + dfop.id + '">';
            _html += '<ul class="nav nav-tabs ll-form-tab">';
            _html += '</ul>';
            _html += '</div>';
            _html += '</div>';

            _html += '<div class="ll-custmerform-designer-layout-area" id="ll_custmerform_designer_layout_area_' + dfop.id + '" ></div>';
            _html += '<div class="ll-custmerform-designer-layout-footer">';
            _html += '<div class="ll-custmerform-designer-layout-footer-item" id="ll_custmerform_tabsEdit_btn_' + dfop.id + '"><i class="fa fa-pencil-square-o"></i><span>编辑选项卡</span></div>';
            _html += '<div class="ll-custmerform-designer-layout-footer-item" id="ll_custmerform_preview_btn_' + dfop.id + '"><i class="fa fa-eye"></i><span>预览表单</span></div>';
            _html += '</div>';
            _html += '<div class="ll-custmerform-designer-layout-center-bg"><img src="' + top.$.rootUrl + '/Content/images/tableform.png" /></div>';

            _html += '</div>';

            _html += '<div class="ll-custmerform-designer-layout-right" id="ll_custmerform_compont_property_' + dfop.id + '"></div>';
        ...
        bind: function ($self) {
            ...
            // 优化滚动条
            $('#ll_custmerform_compont_list_' + dfop.id).mCustomScrollbar({
                theme: "minimal-dark"
            });
            $('#ll_custmerform_designer_tabs_' + dfop.id).mCustomScrollbar({
                axis: "x",
                theme: "minimal-dark"
            });
            $('#ll_custmerform_designer_layout_area_' + dfop.id).mCustomScrollbar({
                theme: "minimal-dark"
            });
            $('#ll_custmerform_compont_property_' + dfop.id).mCustomScrollbar({
                theme: "minimal-dark"
            });
            $('#ll_custmerform_designer_layout_area_' + dfop.id + ' .mCSB_container')[0].dfop = dfop;

            // 编辑选项卡
            $self.find('#ll_custmerform_tabsEdit_btn_' + dfop.id).on('click', function () {
            ...
            // 预览表单
            $self.find('#ll_custmerform_preview_btn_' + dfop.id).on('click', function () {
        ...
        compontinit: function ($self) {// 组件初始化
            var dfop = $self[0]._lrCustmerFormDesigner.dfop;
            var $compontList = $self.find('#ll_custmerform_compont_list_' + dfop.id + ' .mCSB_container');
            $.each($.lrFormComponents, function (i, component) {
                var $component = component.init();
                $compontList.append($component);
            });
            $compontList.find('.ll-custmerform-component').draggable({
                connectToSortable: '#ll_custmerform_designer_layout_area_' + dfop.id + ' .mCSB_container',
                helper: "clone",
                revert: "invalid"
            });

            $('#ll_custmerform_designer_layout_area_' + dfop.id + ' .mCSB_container').sortable({
                opacity: 0.4,
                delay: 300,
                cursor: 'move',
                ...
                stop: function (event, ui) {
                    ...
                    if (!!componttype) {//如果是第一次移入，需要对单元项进行初始化处理
                        var $designer = $compont.parents('.ll-custmerform-designer-layout');

                       
                        $compont.addClass('ll-compont-item').css({ 'width': '100%' });
                        $compont.removeClass('ll-custmerform-component');
                        $compont.removeAttr('data-type');
                        $.lrFormComponents[componttype].render($compont);
                        $compont[0].dfop.id = luoluo.newGuid();
                ...
                start: function (event, ui) {
                    $self.find(".ll-custmerform-designer-layout-center-bg").hide();
                ...
                out: function (event, ui) {
                    if (ui.helper != null) {
                        var $componts = $('.ll-custmerform-designer-layout-area .mCSB_container .ll-compont-item');
                        if ($componts.length <= 1) {
                            if ($componts.length == 1) {
                                if ($componts.find('.ll-compont-value').length == 0) {
                                    $(".ll-custmerform-designer-layout-center-bg").show();
                                }
                            }
                            else {
                                $(".ll-custmerform-designer-layout-center-bg").show();
        ...
        compontbind: function ($self) {
            $self.delegate('.ll-compont-item', 'click', function () {
                var $this = $(this);
                if (!$this.hasClass('active')) {
                    $('.ll-custmerform-designer-layout-area .mCSB_container .ll-compont-item').removeClass('active');
                    $this.addClass('active');
                    if ($('.ll-custmerform-designer-layout').css('padding-right') == '0px') {
                        $('.ll-custmerform-designer-layout').animate({ 'padding-right': '240px', speed: 2000 });
                        $('.ll-custmerform-designer-layout-right').animate({ right: 0, speed: 2000 });
                    }
                    setTimeout(function () {
            ...
            $self.delegate('.ll-compont-remove i', 'click', function () {
                var $compont = $(this).parents('.ll-compont-item');
                $compont.remove();
                if ($('.ll-custmerform-designer-layout-area .mCSB_container .ll-compont-item').length == 0) {
                    $('.ll-custmerform-designer-layout-right').animate({ right: '-240px', speed: 2000 });
                    $('.ll-custmerform-designer-layout').animate({ 'padding-right': '0px', speed: 2000 });
                    $(".ll-custmerform-designer-layout-center-bg").show();
                }
                else {
                    $('.ll-custmerform-designer-layout-area .mCSB_container .ll-compont-item').eq(0).trigger('click');
                }
            });
        },
        // 选项卡事件绑定
        tabbind: function ($self) {
            var dfop = $self[0]._lrCustmerFormDesigner.dfop;
            $self.delegate('#ll_custmerform_designer_tabs_' + dfop.id + ' ul>li', 'click', function () {
                var $this = $(this);
                if (!$this.hasClass('active')) {
                    var $parent = $this.parent();
                    var $self = $this.parents('.ll-custmerform-designer-layout');
        ...
        renderTabs: function ($self) {// 渲染选项卡
            var dfop = $self[0]._lrCustmerFormDesigner.dfop;
            var $tabs = $('#ll_custmerform_designer_tabs_' + dfop.id + ' ul');
            ...
            if (tabsLength <= 1) {
                $self.find('.ll-custmerform-designer-layout-center').removeClass('hasTab');
            }
            else {
                $self.find('.ll-custmerform-designer-layout-center').addClass('hasTab');
        ...
        saveComponts: function ($self) {
            var dfop = $self[0]._lrCustmerFormDesigner.dfop;
            var componts = [];
            var compontsLayout = $('#ll_custmerform_designer_layout_area_' + dfop.id + ' .mCSB_container');
            compontsLayout.find('.ll-compont-item').each(function () {
            ...
            if (dfop._isRenderComponts) {
                ...
                if (dfop._currentComponts.length > 0) {
                    ...
                    for (var i = 0, l = dfop._currentComponts.length; i < l; i++) {
                        ...
                        var $compont = $('<div class="ll-compont-item" ></div>');
                    ...
                    if (dfop.data[i].componts[j].type != 'label') {
                        ...
                        if (table != '' && field != '') {
                            if (!!_data[table + '_' + field]) {
                                luoluo.alert.error('【' + title + '】绑定数据表字段与【' + _data[table + '_' + field] + '】重复！');
                                res = false;
                            }
                            else {
                            ...
                            if (dfop.data[i].componts[j].type == 'girdtable') {
                                if (table == '') {
                                    luoluo.alert.error('【表格项】请绑定数据表！');
                                    res = false;
                                }
                            }
                            else {
                                luoluo.alert.error('【' + title + '】请绑定数据表！');
                                res = false;
                            }
                        }
                    }
