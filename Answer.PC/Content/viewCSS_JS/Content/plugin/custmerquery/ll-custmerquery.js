/*
 * 日 期：2017.11.16
 * 描 述：ISearch 输入搜索框，支持input输入框，数据异步加载，本地查询
 */
(function ($, luoluo) {
    "use strict";
    $.lrcustmerquery = {
        init: function ($self) {
            ...
            $self.parent().append('<div class="luoluo-isearch-panel"  style="max-height:' + dfop.maxHeight + 'px;" ><ul id="learunisearch_' + dfop.id + '" ></ul></div>');
        },
        bind: function ($self) {
    ...
    $.fn.lrcustmerquery = function (op) {
        ...
        //加载数据
        if (!!dfop.url) {
            luoluo.httpAsync('GET', dfop.url, dfop.param, function (data) {
