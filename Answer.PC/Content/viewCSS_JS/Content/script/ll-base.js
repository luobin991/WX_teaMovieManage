...
top.luoluo = (function ($) {
    ...
    var luoluo = {
        ...
        countFileSize: function (size) {
            if (size < 1024.00)
                return luoluo.toDecimal(size) + " 字节";
            else if (size >= 1024.00 && size < 1048576)
                return luoluo.toDecimal(size / 1024.00) + " KB";
            else if (size >= 1048576 && size < 1073741824)
                return luoluo.toDecimal(size / 1024.00 / 1024.00) + " MB";
            else if (size >= 1073741824)
                return luoluo.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
        },
        // 数组复制
        ...
        checkrowofone: function (id) {
            var isOK = true;
            if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
                isOK = false;
                luoluo.alert.warning('您没有选中任何数据项,请选中后再操作！');
            } else if (typeof(id)=="string" && id.split(',').length > 1) {
                isOK = false;
                luoluo.alert.warning('只能选中一个！');
            }
            return isOK;
        },
        // 提示消息栏
        ...
        download: function (options) {
            ...
            if (options.url && options.param) {
                ...
                for (var key in options.param) {
                    var $input = $('<input type="hidden" data-back="backdc" />').attr('name', key).val(options.param[key]);
                    $form.append($input);
                }
